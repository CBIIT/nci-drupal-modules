...
(node). Teachers/editors are any other users modifying that content. Hence there
may be multiple teachers correcting the content, but there can only be one
student per node.
A similar paradigm is that of an author or writer working with a reviewer or
editor.

Diff Different only affects node/% pages and nodes displayed in the "full", as
opposed to "teaser" mode. The module does not change the Diff module pages
